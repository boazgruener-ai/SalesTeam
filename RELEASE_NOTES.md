...
- Second phase of the Target Account & Contact Discovery plan (see PRD 6.20 and the plan file), following v0.30.0's unified safety budget. New `onboarding.html`/`.css`/`.js` - a linear, 9-step explain → enter → confirm wizard capturing the targeting profile a future Discovery scan (Phase 5/6, not yet built) will read: Location, Size + cap, Industry, Priority guidelines, Company Context (reused from Settings), Ideal Customer Profile (reused), Target-contact profile, Competitors to exclude, and Recruiters to exclude. Every field auto-saves to real storage as it's typed (not gated behind Confirm), a free-navigation Back button chains arbitrarily far back through already-visited steps, and reopening the page resumes at the furthest step reached rather than always starting over - all three added directly from the user's own feedback after completing a real run (see below).
- New `storage.js` keys: `targetUniverseConfig` (location/size/industry targeting), `targetContactProfile` (exact titles/title keywords), `accountPriorityGuidelines` (the user's ranked Size/Location/Industry T-shirt-size guideline), `competitorCompanySlugs` and `recruiterCompanySlugs` (both LinkedIn company-page slugs, not names - see below), `companyWebsite`, `onboardingCompletedAt` (gates the wizard), `onboardingProgressStepIndex` (drives resume-where-you-left-off), plus `parseLinkedinCompanySlug()` for turning a pasted LinkedIn URL into a slug.
- **Enforcement**: `background.js`'s `onInstalled` opens the wizard directly on a fresh install (`reason === "install"`). `sidepanel.js` and `target-accounts.js` both show a blocking "Setup isn't finished yet" banner (same `.warning-banner` pattern as the existing missing-API-key banner) linking back to it if `onboardingCompletedAt` is unset. A new "Edit Setup…" entry on the Settings page reopens the wizard any time afterward, pre-filled from existing storage.
- **Identity-based competitor/recruiter exclusion, not name-based** - reported directly: matching by company *name* runs into the exact legal-suffix/short-name/misspelling mess 6.16's resolver already had to solve for a different reason. Both steps instead ask for each company's LinkedIn page **URL**, one per line (this codebase's established list-editing convention), parsed client-side to a slug with immediate per-line ✓/✗ feedback - no LinkedIn visit needed to validate. Kept as two separate lists, not merged, mirroring how the existing Negative Topics feature (6.2) already keeps "Competitor Blocklist" and "Known Recruiting Firms" distinct.
- **Recruiters step pre-seeded with well-known global agencies** (`DEFAULT_RECRUITER_COMPANY_SLUGS`, `storage.js`) so a user only has to add local ones - Adecco, Michael Page, Robert Walters, Hays, Harvey Nash, ManpowerGroup, Heidrick & Struggles, Spencer Stuart, Lionstep, Panda International, Jobgether. Every slug confirmed live against a real LinkedIn company page, not guessed - one real trap found and resolved along the way: "Panda International" has two unrelated same-named LinkedIn pages (an India-based education company and the real, ✓ verified Staffing and Recruiting one in Amsterdam), resolved by industry/location context. The seed applies exactly once (`null` vs. `[]` distinguished in storage), so it never re-appears after a user edits or clears the list.
- New shared `location-picker.js` - the dual-listbox continent/country picker, extracted out of `settings.js` (which had this exact ~350-line widget built once already for Location Filter, 6.13) so the wizard's Location step could mount the identical widget without duplicating it. Settings' own Location Filter now calls into this shared module too - refactored, not rewritten, so its existing behavior (mode/continent changes save immediately; country changes stage behind an explicit Save Countries button) is unchanged.
- New `geo-urn-map.js` and `industry-id-map.js` - confirmed-only country→geoUrn and industry→`industryCompanyVertical` lookup tables (never guessed, same discipline as every other LinkedIn constant in this codebase), each growing incrementally as entries are confirmed live. The wizard's Location and Industry steps only ever offer entries with a confirmed mapping. 10 countries confirmed so far (Switzerland, Germany, Greece, Australia, Belgium, Canada, Costa Rica, United Kingdom, Chile, Colombia) and 9 industries (Financial Services, Technology/Information/Internet, Consumer Services, Food and Beverages, Telecommunications, Hospitals and Health Care, Manufacturing, Pharmaceutical Manufacturing, Utilities) - both confirmed live via the same URL-reading technique already established, catching and correcting two copy-paste duplicate-ID mistakes along the way (Chile/Colombia briefly shared an id; two industry pairs did too) rather than recording them as-is.
- Industry step upgraded from free text to a checkbox list once enough of the catalog existed to make a real picker worthwhile, same pattern as Location's continent checkboxes.
- **Real end-to-end test, reported directly**: the user completed a full run for his wife's sales team's actual requirements. Feedback from that run: a real completion can take "tens of minutes to several hours" (25+ competitor/recruiter LinkedIn lookups alone was the slowest part) - addressed by the continuous auto-save, free Back navigation, and resume-at-step described above, plus a new optional Company Website field (`companyWebsite`) as groundwork for a future AI-assisted drafting pass (not built yet - recorded in the plan for later, needs its own design work).
